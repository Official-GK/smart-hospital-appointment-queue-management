...
const AppointmentManagement = ({ defaultStaffId = 'STF-001', refreshTrigger = 0 }) => {
  ...
  const handleBookingDoctorOrDateChange = async (newDocId, newDate) => {
    ...
    setBookingForm((prev) => ({
      ...prev,
      doctorId: newDocId,
      departmentId: docObj ? docObj.department_id : prev.departmentId,
  ...
  const displayedAppointments = appointments.filter((apt) => {
    ...
    return (
      ...
      (apt.appointment_id && apt.appointment_id.toLowerCase().includes(q)) ||
      (apt.patient_id && apt.patient_id.toLowerCase().includes(q)) ||
      (apt.doctor_name && apt.doctor_name.toLowerCase().includes(q)) ||
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* Reschedule / Modification Modal using AppointmentModal, Input & Button */}
      <AppointmentModal
      ...
      {/* Appointment Cancellation Modal using AppointmentModal, Input & Button */}
      <AppointmentModal
      ...
      {/* Slot Inventory & Released Capacity Modal using AppointmentModal & Table */}
      <AppointmentModal
      ...
      {/* Audit History Modal using AppointmentModal & Badge */}
      <AppointmentModal
        ...
        maxWidth="640px"
        footer={<Button variant="secondary" onClick={() => setHistoryModalOpen(false)}>Close</Button>}
      >
                ...
                <div key={record.history_id || index} className="timeline-item">
                  ...
                  <div className="timeline-content">
                    ...
                    <div className="timeline-meta">
                      <span><strong>Staff ID / Actor:</strong> {record.changed_by || 'Staff Member'}</span>
                    </div>
      ...
      {/* Book Appointment Modal using AppointmentModal, Input, Button */}
      <AppointmentModal
...
AppointmentManagement.propTypes = {
  defaultStaffId: PropTypes.string,
};
